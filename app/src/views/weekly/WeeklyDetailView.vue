...
const articleSource = computed(() => {
  return `weekly/${articleId.value}/content`;
});

const loadWeeklyMetadataAsync = async (locale?: string) => {
  const targetLocale = locale ?? currentLocale.value;
  const api = `/content/weekly/${articleId.value}/metadata.${targetLocale}.json`;
  ...
  <body-block>
    ...
    <div class="content-paper relative">
      ...
      <div class="absolute top-10 left-5">
        <anchor href="/weekly" route-name="weekly" :title="$t('media-weekly')" mode="h5">
